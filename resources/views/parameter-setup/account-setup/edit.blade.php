    ...
    <div class="row">
        <div class="col-md-6">
            <div class="ibox-content">
                <h2 class="font-bold">Account Modify</h2>
                <div class="row">
                    <div class="col-lg-12">

                        <form id="user-create-form" method="POST" enctype="multipart/form-data" action="{{ route('parameter_setup.account_setup.update', [$account_info->id]) }}">
                            @csrf
                            
                           <div class="form-group">
                                <label>Account Name</label>
                                <input type="text" name="name" value="{{ $account_info->name }}"  id="name" required class="form-control">
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" required onchange="findParentAccountNo()" name="acc_level" id="acc_level">
                                    ...
                                    @for($i=1; $i<=7; $i++)
                                        <option value="{{$i}}"  @if($account_info->account_level == $i) {{ "selected" }} @endif >{{ $i }}</option>
                                    @endfor
                                </select>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" required onchange="findParentAccountNo()" name="acc_types" id="acc_types">
                                    ...
                                    @foreach($account_types as $account_type)
                                        <option value="{{ $account_type->id }}" @if($account_info->account_type_id == $account_type->id ) {{ "selected" }} @endif >{{ $account_type->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" name="allow_manual_transction" id="allow_manual_transction">
                                    <option value="1" @if($account_info->allow_manual_transaction == 1) {{ "selected" }} @endif>Yes</option>
                                    <option value="0"  @if($account_info->allow_manual_transaction == 0) {{ "selected" }} @endif>No</option>
                                </select>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" name="allow_negetive_transction" id="allow_negetive_transction">
                                    <option value="1" @if($account_info->allow_negative_balance == 1) {{ "selected" }} @endif >Yes</option>
                                    <option value="0" @if($account_info->allow_negative_balance == 0) {{ "selected" }} @endif>No</option>
                                </select>
                            </div>
